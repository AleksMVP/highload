...
На менее нагруженных серверах можно использовать HDD объединенные в RAID-0 или RAID-5. Помимо этого,  стоит использовать CDN для кэширования контента и обеспечения качества для пользователей, которые находятся далеко от основных серверов. SSD тоже можно объединять в RAID5 или RAID-10 массивы, но скорее всего мы упремся в пропускную способность сети чем в скорость SSD. Все таблицы, за исключением таблицы пренадлежности пользователя к конкретной группе, хранятся в postgres. Таблица соответствия пользователя и группы хранится в Redis. Так же в Redis будем хранить информацию о сессиях пользователей и соответствие видео с серверами.

## Принцип сохранения видео:
Популярные видео будут распределятся по хранящим CDN серверам. Популярность будем определять исходя из количество подписчиков пользователя, который публикует видео. Нам нужно иметь минимум 3 копии видео и эти копии должны располагаться на разных серверах, имеющие разное географическое расположение. 

## Принцип нахождения видео:
Определение CDN сервера будет осущестляться на backend сервере. В Redis будет хранится таблица соответствия контента с серверами. Т.к. мы имеем несколько копий контента на разных серверах, то сервер будет выбиратся с помощью алгоритма RoundRobin.
