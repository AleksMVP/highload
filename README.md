...
На менее нагруженных серверах можно использовать HDD объединенные в RAID-0 или RAID-5. Помимо этого,  стоит использовать CDN для кэширования контента и обеспечения качества для пользователей, которые находятся далеко от основных серверов. SSD тоже можно объединять в RAID5 или RAID-10 массивы, но скорее всего мы упремся в пропускную способность сети чем в скорость SSD. Все таблицы, за исключением таблицы пренадлежности пользователя к конкретной группе, хранятся в postgres. Таблица соответствия пользователя и группы хранится в Redis. Так же в Redis будем хранить информацию о сессиях пользователей.

## Принцип сохранения видео:
Популярные видео будут распределятся по хранящим CDN серверам. Популярность будем определятся исходя из количество подписчиков пользователя, который публикует видео. Нам нужно иметь минимум 3 копии видео и эти копии должны располагаться на разных серверах, имеющие разное географическое расположение. Определение сервера будет осуществлятся по формуле: id видео % количество серверов. Нахождение следующей копии видео будет вычисляться по формуле: (номер первого сервера + количество серверов / количество копий) % количество серверов. Адреса серверов будут определяться на клиенте, если сервер не отвечает будет делаться запрос на следующий CDN сервер, но не больше 2х раз.
...
Шардирование БД будет осуществляться по полю id в таблице users также для больших таблиц videos, comments нужно будет применить парционирование по полю id.
...
# Расчет оборудования
* Самый популярный контент будет храниться на CDN серверах. Предположим, что его количество 3% от всего контента -> 43 ПБ * 0,03 = 1.29 ПБ или 1,290 ТБ. Из расчета, что самым оптимальным вариантом является SSD диск на 4 ТБ нам понадобится 1,290 ТБ / 4 ТБ = 323 таких диска. Если принять, что пиковая нагрузка за одним видео = 1% от пиковой нагрузки = 23,200 Гбит/сек / 100 = 232 Гбит/с, то нам понадобится как минимум три сервера с двумя 50 Гбит/сек карточками, на которых будет хранится копия этого видео. Следовательно нам понадобится в три раза больше SSD, если в каждый сервер ставить по 10 SSD, то получится 100 CDN серверов.
* Также весь контент, в том числе популярный, будет храниться в "хранилках". На этих серверх будут стоять HDD диски в RAID5. На каждом сервере будет располагаться две 10 Гбит/с карты и 30 дисков по 8 ТБ c учетом RAID. 43,000 ТБ / (20 * 8 ТБ) = 268 серверов, а с учетом 3х копий 804 сервера. 
* Перед хранилками и фронтендами будут стоять кеширующие CDN сервера. Это нужно для следующего:
  1. Если непопулярный контент станет вдруг очень популярным, он закешируется на CDN серверах и проблем не возникнет.
  2. Обеспечение качества для жителей регионов расположеных далеко от серверов.
...
<table>
  ...
  <tr>
    ...
    <td>4</td>
    <td>32</td>
    <td>HDD RAID5 по 3 диска</td>
    <td>(30 - 10) x 8 TБ</td>
    <td>2 x 10 Гбит/с</td>
    <td>804</td>
  </tr>
  <tr>
    <td>Сервера Postgres</td>
    <td>64</td>
    <td>256</td>
    <td>SSD PCIE 4.0</td>
    <td>4 x 1 TБ</td>
